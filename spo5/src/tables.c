...
symb_table_t* push_var(char* name, symb_table_t* symb_table) {
    return push_var_typed(name, NULL, symb_table);
}

user_type_t* find_user_type(char* name, user_type_t* list) {
    while (list) {
        if (strcmp(name, list->name) == 0) return list;
        list = list->next;
    }
    return NULL;
}
...
type_member_t* push_field_member(char* name, struct type* type, type_member_t* members, bool inherited) {
    type_member_t* m = malloc(sizeof(type_member_t));
    m->kind = MEMBER_FIELD;
    m->name = strdup(name);
    m->type = type;
    m->method = NULL;
...
type_member_t* push_method_member(struct func_sig* method, type_member_t* members, bool inherited) {
    type_member_t* m = malloc(sizeof(type_member_t));
    m->kind = MEMBER_METHOD;
    m->name = strdup(method->name);
    m->type = method->return_type;
    m->method = method;
    m->inherited = inherited;
    m->next = members;
    return m;
