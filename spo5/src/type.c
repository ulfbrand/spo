...
type_t* create_non_array_type(char* label) {
    ...
    if(strcasecmp(label, "BOOL") == 0) {
        curr_type->kind = BUILTIN;
        curr_type->builtin = MY_BOOL;
    } else if (strcasecmp(label, "BYTE") == 0) {
        curr_type->kind = BUILTIN;
        curr_type->builtin = MY_BYTE;
    } else if (strcasecmp(label, "INT") == 0) {
        curr_type->kind = BUILTIN;
        curr_type->builtin = MY_INT;
    } else if (strcasecmp(label, "UINT") == 0) {
        curr_type->kind = BUILTIN;
        curr_type->builtin = MY_UINT;
    } else if (strcasecmp(label, "LONG") == 0) {
        curr_type->kind = BUILTIN;
        curr_type->builtin = MY_LONG;
    } else if (strcasecmp(label, "ULONG") == 0) {
        curr_type->kind = BUILTIN;
        curr_type->builtin = MY_ULONG;
    } else if (strcasecmp(label, "CHAR") == 0) {
        curr_type->kind = BUILTIN;
        curr_type->builtin = MY_CHAR;
    } else if (strcasecmp(label, "STRING") == 0) {
        curr_type->kind = BUILTIN;
        curr_type->builtin = MY_STRING;
    } else if (strcasecmp(label, "VOID") == 0) {
        curr_type->kind = BUILTIN;
...
type_t* parse_type(ast_t* ast, struct file_defs* defs) {
    ...
    if (curr_type->kind == CUSTOM && defs && !find_user_type(curr_type->custom, defs->user_types)) {
        push_err(&defs->err, "%s: Unknown type `%s` (line %d, pos %d)", ERROR, curr_type->custom, ast->children[0]->line, ast->children[0]->pos);
    }
    return curr_type;
