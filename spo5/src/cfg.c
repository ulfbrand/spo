...
cfg_block_t* parse_var(ast_t* ast, cfg_block_t* prev_block, file_defs_t* defs) {
    ...
    for (uint32_t i = 1; i <  ast->children_cnt; ++i) {
        if (!get_var(ast->children[i]->children[0]->label, last_block->symb_table)) {
            last_block->symb_table = push_var_typed(ast->children[i]->children[0]->label, type, last_block->symb_table);
        } else {
            push_err(&defs->err, "%s: Redeclaration of variable `%s` (line %d, pos %d)", ERROR, ast->children[i]->children[0]->label, ast->children[i]->children[0]->line, ast->children[i]->children[0]->pos);
        }
...
void inherit_members(user_type_t* type, file_defs_t* defs, char* filename) {
    if (!type || type->resolved) return;
    type->resolved = true;
    if (!type->parent_name) return;
    user_type_t* parent = find_user_type(type->parent_name, defs->user_types);
    if (!parent) {
        push_err(&defs->err, "%s: Unknown parent type `%s` for `%s` in %s", ERROR, type->parent_name, type->name, filename);
        return;
    }
    type->parent = parent;
    inherit_members(parent, defs, filename);
    for (type_member_t* m = parent->members; m; m = m->next) {
        if (!find_type_member(type, m->name)) {
            if (m->kind == MEMBER_FIELD) type->members = push_field_member(m->name, m->type, type->members, true);
            else type->members = push_method_member(m->method, type->members, true);
        }
    }
}

user_type_t* parse_user_type_def(ast_t* type_def, file_defs_t* defs, char* filename) {
    if (strcmp(type_def->label, "TYPE_DEF") != 0) return NULL;
    char* type_name = type_def->children[0]->label;
    if (find_user_type(type_name, defs->user_types)) {
        push_err(&defs->err, "%s: Redefinition of type `%s` in %s", ERROR, type_name, filename);
        return NULL;
    }
    user_type_t* t = malloc(sizeof(user_type_t));
    t->name = strdup(type_name);
    t->parent_name = NULL;
    t->parent = NULL;
    t->members = NULL;
    t->resolved = false;
    t->next = defs->user_types;
    defs->user_types = t;

    uint32_t start = 1;
    if (type_def->children_cnt > 1 && strcmp(type_def->children[1]->label, "BASE_TYPE") == 0) {
        if (type_def->children[1]->children_cnt > 0) t->parent_name = strdup(type_def->children[1]->children[0]->label);
        start = 2;
    }
    for (uint32_t i = start; i < type_def->children_cnt; ++i) {
        ast_t* member = type_def->children[i];
        if (strcmp(member->label, "FIELD_DEF") == 0) {
            char* name = member->children[1]->label;
            if (find_type_member(t, name)) {
                push_err(&defs->err, "%s: Redeclaration of field `%s.%s` in %s", ERROR, t->name, name, filename);
                continue;
            }
            t->members = push_field_member(name, parse_type(member->children[0], defs), t->members, false);
        } else if (strcmp(member->label, "METHOD_DEF") == 0) {
            func_sig_t* method = parse_signature(member->children[0], defs, filename);
            char* old_name = method->name;
            size_t len = strlen(t->name) + strlen(old_name) + 2;
            method->name = malloc(len);
            snprintf(method->name, len, "%s.%s", t->name, old_name);
            free(old_name);
            t->members = push_method_member(method, t->members, false);
            method->next = defs->signatures;
            defs->signatures = method;
            if (method->defined) method->cfg = parse_body(member->children[0]->children[1], defs, method, filename);
        }
    }
    return t;
}
...
file_defs_t* parse_ast(ast_t** ast, uint32_t cnt, char* output_path, bool need_ot) {
    ...
    for (uint32_t i = 0; i < cnt; ++i) {
        for (uint32_t j = 0; j < ast[i]->children_cnt; ++j) {
            if (strcmp(ast[i]->children[j]->label, "TYPE_DEF") == 0) {
                parse_user_type_def(ast[i]->children[j], defs, ast[i]->filename);
            }
        }
    }
    for (user_type_t* t = defs->user_types; t; t = t->next) {
        inherit_members(t, defs, ast && cnt ? ast[0]->filename : "<source>");
    }
    for (uint32_t i = 0; i < cnt; ++i) {
        for (uint32_t j = 0; j < ast[i]->children_cnt; ++j) {
            ast_t* func_def = ast[i]->children[j];
            if (strcmp(func_def->label, "TYPE_DEF") == 0) continue;
            ...
            switch(status) {
                ...
                case NOT_FOUND:
                    ...
                    if (curr_sig->defined) curr_sig->cfg = parse_body(func_def->children[1], defs, curr_sig, ast[i]->filename);
                    break;
            }
        }
    }
