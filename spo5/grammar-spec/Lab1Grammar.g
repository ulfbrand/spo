...
typeDef     : TYPE_KEYWORD ID (COLON ID)? LEFT_FBRACE typeMember* RIGHT_FBRACE
              -> ^(TYPE_DEF ID ^(BASE_TYPE ID)? typeMember*)
            ;

typeMember  : typeRef ID SEMICOLON -> ^(FIELD_DEF typeRef ID)
            | funcDef -> ^(METHOD_DEF funcDef)
            ;
...
postfixOp       : memberAccess | indexer | call
                ;
